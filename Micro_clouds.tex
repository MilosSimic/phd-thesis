...
%%TODO: Videti kako ovo rekurzivno srediti!!
\begin{algorithm}
	\SetKwFunction{printlcs}{\textsc{Print}-LCS}
	\Indm\printlcs{$b,X,i,j$}\\
	\Indp
	\If{$i=0$ or $j=0$}{\KwRet{}}
...
\begin{algorithm}[H]
	...
	\uIf{id = null}{
		...
		data $\leftarrow$ findRequest(id)\\
		\uIf{data.intersection(request.payload.topology)}{
			return true;
		}
		return false;
	}
...
%%TODO: DODATI OPISE ZA SVKOG UCESNIKA
\begin{enumerate}[start=1,label={(\bfseries \arabic*)}]
	\item \textbf{User} sends a list request to State service;
