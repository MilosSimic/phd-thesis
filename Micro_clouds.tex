...
%
Frontend services model should be packed in some standard way~\ref{sec:virtualization_techniques} and deployed in the wild, as an event-driven applications, with the subscription policy to message streams using topics~\cite{inproceedingsBeck}. The processing strategy is in the developer's hands, depending on the nature of the use-case. 

If an services are subscribed to some stream or topic, it is naturally that events apear in their stream. There are basically two strategies for building a large-scale time-ordered event system:
...
\begin{enumerate}[start=1,label={(\bfseries \arabic*)}]
	...
	\item \textbf{services} or applications that would operate over standard request-response model or some variantion of that protocol;
...
\mathtt{istorage} \to \mathtt{service}{:}\mathit{resp}(Boolean).\\
& \hspace{4mm}
\left\{
\begin{array}{@{}l@{}}
\mathtt{service}\to\mathtt{log}{:} \mathit{done}(\mathsf{String}).\tend\\
\mathtt{service}\to\mathtt{nodes}{:}\mathit{ids}(\T_4).
\mathtt{nodes}\to \mathtt{state}{:}\mathit{rsrvd}().\G''
\end{array} \right.
\end{align*}
...
\left\{
\begin{array}{@{}l@{}}
\mathtt{service}{!}\mathit{mutate}().\mathtt{service}{!}\mathit{create}(\T_3).\mathtt{service}{?}\mathit{ok}().\tend\\
...
\left\{
\begin{array}{@{}l@{}}
\mathtt{user}{?}\mathit{mutate}().\mathtt{user}{?}\mathit{create}(\T_3).\mathtt{user}{!}\mathit{ok}().\ST'\\
\mathtt{user}{?}\mathit{quit}().\tend \\
\end{array} \right.
\end{align*}
where
...
\hspace{20mm}
\ST' = &
\mathtt{istorage}{!}\mathit{try}(\T_3).\mathtt{istorage}{?}\mathit{resp}(Boolean).\\
&
{+}
\left\{
\begin{array}{@{}l@{}}
\mathtt{nodes}{!}\mathit{ids}(\T_4).\mathtt{nodes}{?}\mathit{rsrvd}().\ST''\\
\mathtt{log}{!}\mathit{done}().\tend\\
\end{array} \right.\\
%\end{align*}
%
%\begin{align*}
\hspace{-8mm}
\ST'' = & 
\mathtt{mstorage}{!}\mathit{store}(\T_3).\mathtt{mstorage}{?}\mathit{saved}().\\ & \hspace{-8mm}
...
\mathtt{service}{?}\mathit{store}(\T_3).\mathtt{istorage}{!}\mathit{store}(\T_3).\\ & \hspace{-8mm}
...
\hspace{-8mm}
\ST_\mathtt{istorage} =  &
\mathtt{service}{?}\mathit{try}(\T_3).\mathtt{service}{!}\mathit{resp}(Boolean).\\ & \hspace{-8mm}
\mathtt{mstorage}{!}\mathit{store}(\T_3).\mathtt{mstorage}{!}\mathit{saved}()\\
%\end{align*}
%
%\begin{align*}
\hspace{-8mm}
\ST_\mathtt{log} =  &
\mathtt{iservice}{?}\mathit{done}(\T_3).\tend\\
%\end{align*}
%
%\begin{align*}
\ST_\mathtt{nodes} =  &
