...
We can make this test a little bit faster. If we first test does the same topology name is already present in the idempotency store, that lookup will spare us unnecessary comparison on sets.
We can go a little bit further and use CRDTs and SEC to store and replicate data on copies of the services that are required for the idempotency test.
