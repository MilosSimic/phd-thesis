...
Industry is migrating much of their applications to the cloud, because CC offers to scale their computing resources as per their usage~\cite{LiZJLZLGGS19}. Microservices are small units of code that follos UNIX philosophy~\say{do one thing and do it well}~\cite{krause2015microservices}, and they communicate over well defined API~\cite{DragoniGLMMMS16}.
...
%
In this section we are going to shortly describe models that are similar to the DS, and as such they may be the source of confusion. Section~\ref{sec:decentralized_systems} describe decentralized computing compared to DS. Section~\ref{sec:parallel_computing} describes parallel computing compared to DC.

\subsection{Decentralized systems}\label{sec:decentralized_systems}
%
Decentralized systems are similar to DS, in technical sense they are still DS. But if we take closer look, these systems \textbf{should not} be owned by single the entity. CC for example is example of DS, but centralized by the owner like AWS or Google because all computation needs to be moved to big DCs, which introduces a high latency in the system~\cite{HossainRH18}.

By today standards, if people are talking about decentralyzed systems, they usually mean on blockchain technology~\cite{LeibleSSG19}. But even if this technology is run in the cloud, it is loosing the decentralized feature. This is the caveat people needs to be aware. These systems are facing different issues, because any participent in the sustem might be malicious and they need to handle this case. Nontheless, CC can and should be decentralized in a sense that some computation can happend outside CC DCs, closer to the sources of data.
%
%
\subsection{Parallel computing}\label{sec:parallel_computing}
%
DC and parallel computing seems like models that overlap, and that may share some features. Broadly speaking, this might be true~\cite{Vera16}. But distinguished between the two can be presented as follows: in parallel computing all processor units have acces to the shared memory, while in DC all processors have their own memory. Figure~\ref{fig:fig4} visually summarize the differences between DC $(up)$ and parallel computing $(down)$.
...
\begin{figure}[H]
	...
	\end{center}
	\vspace{-0.6cm}
	\caption{Difference between DC and parallel computing.}
	\label{fig:fig4}
\end{figure}

Parallel computing is ofet used strategy with problems, that due to their nature or constraints must be done on multi-core machines simultaneously~\cite{0072397}.
%
%
